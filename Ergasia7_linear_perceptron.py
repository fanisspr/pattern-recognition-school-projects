...
# X_val = np.concatenate([X[(i+30):(i+40), :] for i in [0,50,100]])
X_test = np.concatenate([X[(i+40):(i+50), :] for i in [0,50,100]])

y_train = np.concatenate([y[i:(i + 40)] for i in [0, 50, 100]])
# Y_val = np.concatenate([Y[(i+30):(i+40)] for i in [0,50,100]])
...
lin_percep = Perceptron(random_state=0, max_iter=num_epochs, eta0=lr, early_stopping=True, validation_fraction=0.3, n_iter_no_change=5, verbose=1)
